...
function parseCertificateChain(cert) {
  ...
  while (current && current.subject) {
    ...
    // Stop if we've reached a self-signed cert (root)
...
function calculateGrade(result) {
  ...
  if (result.certificate) {
    ...
    // Check key size (only for RSA - ECDSA 256/384 bit is strong)
    // ECDSA keys are typically 256 or 384 bits, RSA keys are 2048+
    if (result.certificate.bits && result.certificate.bits > 512 && result.certificate.bits < 2048) {
      // This is likely RSA with a weak key
      score -= 20;
